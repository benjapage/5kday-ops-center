# 5KDay Ops Center — Documento de Mejoras V3 (Final)
...
Este documento contiene 24 mejoras y correcciones organizadas por sección, listas para implementar en Claude Code.
...
## 7. CORRECCIONES POST-REVIEW (tras testeo con datos reales)

### Corrección 21 — Meta del mes: quitar el box
- Sacar el card/box que contiene la meta del mes en el header.
- Que quede como **texto integrado** en el header, sin borde ni fondo.
- Solo mostrar: label "META DEL MES" sutil arriba, el número actual en verde destacado (ej: US$ 1.941,34), una barra de progreso fina, y el objetivo (US$ 60.000) con opacidad más baja o un efecto sutil.
- **Sacar el desglose** (Ads, Tools, Equipo, Neto) de la meta — eso ya está en el módulo financiero.
- No debe empujar el contenido hacia abajo. Debe quedar compacto y alineado horizontalmente con "Buenos días, Benjamín".

### Corrección 22 — Vinculación visible entre activos (CRÍTICA)
La cadena de relación entre activos no es visible. Hay que arreglar:

1. **Tabla de Números WA**: la columna "BM ID" debe mostrar el **nombre del BM** (no el ID truncado). Clickeable para navegar al BM.
2. **Cuentas Publicitarias**: cada CP debe mostrar a qué **BM pertenece** como badge o columna visible. Cuando dice "BM vinculado restringido — activos en riesgo", debe especificar **cuál BM** está restringido (ej: "BM Las Recetas de Ana — restringido").
3. **BMs**: cada BM debe mostrar a qué **perfil** pertenece. Badge con el nombre del perfil.
4. **Perfiles — bug de base de datos**: el error `Could not find the 'profile_function' column of 'meta_profiles'` indica que falta agregar la columna `profile_function` a la tabla `meta_profiles` en Supabase. Correr migración:
```sql
ALTER TABLE meta_profiles ADD COLUMN profile_function TEXT;
```

La cadena completa debe ser visible en cada nivel:
**Número → muestra su BM → BM muestra su Perfil → Perfil muestra su función**

### Corrección 23 — Financiero: gráfico como pestaña por defecto
- En el módulo Financiero, la pestaña activa por defecto en la parte de abajo debe ser **"Gráfico"** (barras de Ingresos, Gastos y Profit diario).
- Actualmente muestra la lista/tabla de inversiones por defecto, que es menos visual.
- La tabla queda accesible en su pestaña, pero no es la vista principal al entrar.

### Corrección 24 — Vinculación Drive ↔ Ofertas (flujo completo)
Actualmente desde Pipeline > Oferta > "Abrir carpeta de creativos en Drive" dice que vincules desde Integraciones, pero en Integraciones no hay forma de hacerlo. Falta el flujo completo:

1. **En Integraciones > Google Drive**: agregar sección "Carpetas por oferta" donde se puede asignar una carpeta específica de Drive a cada oferta del Pipeline. Ej: "Esqueletización Botánica → /Anuncios/Esqueletización/". Usar un file picker de Drive para seleccionar la carpeta.

2. **En Pipeline > Detalle de oferta**: una vez vinculada la carpeta, el botón "Abrir carpeta de creativos en Drive" abre directamente esa carpeta. El banco de creativos de la oferta muestra los archivos de la carpeta (thumbnails de imágenes/videos).

3. **En Pipeline > Banco de creativos general**: mostrar creativos de todas las carpetas vinculadas, agrupados por oferta. No solo el botón "+ Agregar creativo" manual, sino que se alimente automáticamente de Drive.

Flujo: conectás Drive en Integraciones → asignás carpetas a ofertas → los creativos aparecen automáticamente en Pipeline y en el dashboard.

---

## 8. INTEGRACIÓN UTMify (próxima prioridad)

UTMify se puede vincular con Claude/la app. Cuando esté disponible:
- **UTMify → Módulo Financiero**: facturación real por oferta (Shopify + WhatsApp)
- **Tracking de conversiones**: datos reales de performance por oferta
- **Alimenta el dashboard**: gráfico de barras y cuadrantes financieros con datos automáticos
- Elimina la necesidad de carga manual de ingresos
...
|-----------|---------|---------|
| 🔴 Alta | 1, 2, 3, 4, 5, 6, 7, 20, 21 | Dashboard principal usable y visual |
| 🔴 Alta | 22 | Vinculación entre activos (crítica para operación) |
| 🟡 Media | 8, 9, 10, 11, 12, 14, 23 | Activos, Pipeline y Financiero funcionales |
| 🟢 Normal | 13, 15, 16, 17 | Módulos secundarios mejorados |
| 🔵 Fase 2 | 18, 19, 24 | Integraciones Drive, Notion, expansión |
| 🔵 Fase 2 | UTMify | Datos financieros automáticos |
...
- **Bug crítico**: falta la columna `profile_function` en la tabla `meta_profiles` (Corrección 22).
- Implementar los cambios de forma incremental, empezando por el dashboard principal (Cambios 1-10, 21) y luego las secciones internas.
- La corrección 22 (vinculación entre activos) es urgente — afecta la usabilidad real de la sección Activos Meta.
