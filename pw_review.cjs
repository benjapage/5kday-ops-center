...
(async () => {
  ...
  await page.fill('input[type="email"], input[placeholder*="email"], input[name="email"]', 'benjapagellafangio@gmail.com');
  await page.fill('input[type="password"], input[placeholder*="ontraseña"], input[name="password"]', 'gatpingi');
  await page.screenshot({ path: `${baseDir}/ss_01_login_filled.png`, fullPage: true });
  ...
  const metaText = await page.evaluate(() => document.body.innerText.substring(0, 500));
  ...
  const financialText = await page.evaluate(() => document.body.innerText.substring(0, 500));
  ...
  const pipelineText = await page.evaluate(() => document.body.innerText.substring(0, 500));
  ...
  const teamText = await page.evaluate(() => document.body.innerText.substring(0, 500));
